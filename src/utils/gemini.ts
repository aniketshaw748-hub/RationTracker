...
// Shared request helper: sends the key as a header (not a query param, which
// leaks into logs/proxies) and returns the first candidate's text.
...
async function callGeminiForRecipe(parts: object[]): Promise<ParsedRecipe> {
...
"""`;
  ...
  return callGeminiForRecipe([{ text: prompt }]);
}

const UNIT_RULES = `Convert ingredients into their corresponding units: 'g' for dry weight, 'ml' for liquids, 'count' for discrete units (like eggs, bread slices, whole vegetables).
Be precise with the ingredient names (e.g. use standard names like "Rice", "Sugar", "Milk").`;
...
export async function parseRecipeFromUrl(url: string): Promise<ParsedRecipe> {
  ...
  if (isYouTube) {
    // Gemini can ingest YouTube videos directly as file input, so the recipe
    // is extracted from the actual video content.
    const prompt = `Watch this cooking video and extract the recipe shown in it.
Identify the dish name, servings, step-by-step instructions, and ingredients with their quantities as demonstrated in the video.
${UNIT_RULES}`;
  ...
  // Non-YouTube links can't be watched by Gemini; the recipe is inferred
  // from the dish named in the URL, not extracted from the video.
  const prompt = `A user has provided this cooking video link: "${url}".
You cannot watch this video. Instead, infer the dish from the URL path or domain name and generate a highly authentic, traditional recipe for it.
